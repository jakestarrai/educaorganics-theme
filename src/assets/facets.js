...
class FacetMoreLess extends HTMLElement {
  constructor() {
    ...
    this.clickCount = 0;
    this.filterItems = JSON.parse(this.querySelector('[data-filter-items]').innerHTML);
    ...
    const selectedElementList = document.querySelector('.filter-selected-element-list');

    function handleChanges(mutationsList, observer) {
      for (const mutation of mutationsList) {
        if (mutation.type === 'childList') {
          if (selectedElementList) {
            const activeFiltersElement = selectedElementList.querySelector('[data-active-filters]');

            if (activeFiltersElement && selectedElementList.classList.contains('hidden')) {
              selectedElementList.classList.remove('hidden');
              selectedElementList.classList.add('flex');
            } else if (!activeFiltersElement && selectedElementList.classList.contains('flex')) {
              selectedElementList.classList.add('hidden');
              selectedElementList.classList.remove('flex');
            }
          }
        }
      }
    ...
    if (selectedElementList) {
      ...
      observer.observe(selectedElementList, {
        childList: true,
        subtree: true,
      });
    }
    ...
    this.addEventListener('click', function (e) {
      ...
      if (this.filterItems?.filter_label === 'Color') {
        this.itemsToFilter = this.filterItems.filter_values.map(
          (item, index) => `
          <div class="py-3 relative" data-filter-element>
      ...
      this.insertAfterEl = this.parentElement.querySelector(`[data-filter-element]`);
      ...
      if (e.target.dataset.id === 'less') {
        ...
        this.items = this.parentElement.querySelectorAll(`[data-filter-element]`);
...
class FacetFiltersForm extends HTMLElement {
  ...
  static renderProductCount(html) {
    const updatedContainer = new DOMParser().parseFromString(html, 'text/html');
    const count = updatedContainer.getElementById('ProductCountDesktop').innerHTML;

    const totalProductsCount = updatedContainer.querySelector('[data-total-products-quantity]').dataset
      .totalProductsQuantity;

    const container = document.getElementById('ProductCount');
    const containerDesktop = document.getElementById('ProductCountDesktop');
    const productsContainer = document.querySelector('[data-total-products-quantity]');

    if (container) container.innerHTML = count;

    if (containerDesktop) containerDesktop.innerHTML = count;

    productsContainer?.setAttribute('data-total-products-quantity', totalProductsCount);
  }
  ...
  static renderFilters(html, event) {
    ...
    facetsToRender.forEach((element) => {
      const filterSlideContainer = element.querySelector('[data-filter-slide-container]');

      document.querySelector(
        `.js-filter[data-index="${element.dataset.index}"] [data-filter-slide-container]`
      ).innerHTML = filterSlideContainer.innerHTML;
    });
