...
MobileNav.prototype = {
  initStructure() {
    ...
    this.menuContainer = this.container.querySelector(this.options.menuContainer);
  },
  attachEvents() {
    ...
    this.outsideClickHandler = (e) => {
      if (this.isOpened()) {
        ...
        if (!target.closest(this.options.menuOpener) && !this.menuContainer.contains(target)) {
          this.hide();
          const hasInertAttr = this.menuContainer.hasAttribute('inert');

          if (!hasInertAttr) {
            this.menuContainer.setAttribute('inert', '');
          }
        }
      }
    };
