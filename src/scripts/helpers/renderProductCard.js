...
export class RenderProduct {
  ...
  _showSpinnerContainer() {
    this.spinnerHolder.classList.remove('hidden');
  }

  _hideSpinnerContainer() {
  ...
  async fetchNextPage() {
    ...
    const urlParamsSize = Array.from(urlParams.entries()).length;

    const urlParamsString = urlParamsSize > 0 ? `&${urlParams.toString()}` : '';
    let nextPage;

    if (this.container.dataset.sectionType === 'collection') {
      nextPage = await fetch(
        `/collections/${this.container.dataset.collectionHandle}?page=${this.page}&${urlParamsString}`
      );
    } else {
      nextPage = await fetch(
        `${window.location.pathname}?page=${this.page}&q=${this.container.dataset.searchTerm}&${urlParamsString}`
      );
    }
    ...
    if (variant) {
      if (variant.compare_at_price > variant.price) {
        priceContainer.innerHTML = `
          <ins class='no-underline'>${formatMoney(variant.price, theme.moneyFormat)}</ins>
  ...
  changeCardQuickViewLink(productCardQuickAdd, variantId) {
    ...
    const newProductUrl = productUrl.includes('?variant=')
      ? productUrl.replace(/(\?variant=)\d+/g, `?variant=${variantId}&view=quick-card`)
      : `${productUrl}?variant=${variantId}&view=quick-card`;
