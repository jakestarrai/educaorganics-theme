...
class App {
  ...
  responsiveFluidIframe() {
    document.querySelectorAll('.prose iframe').forEach((iframe) => {
      const fluidHtml = document.createElement('div');

      fluidHtml.classList.add('fluid-iframe', 'relative', 'h-0', 'pt-[56.25%]');
  ...
  initMobileNav() {
    window.onload = () => {
      new MobileNav('body', {
        ...
        menuDrop: '.menu',
        menuContainer: '.menu-drawer',
        hideOnClickOutside: true,
      });

      // toggle menu-drawer attr inert if clicks on menu__opener button
      const menuDrawer = document.querySelector('.menu-drawer');

      if (menuDrawer) {
        const menuButtonsOpener = document.querySelectorAll('.menu__opener');

        const toggleInert = () => {
          const hasInertAttr = menuDrawer.hasAttribute('inert');
        ...
        menuButtonsOpener.forEach((item) => {
          item.addEventListener('click', toggleInert);
        });
      }
    };
  }
  ...
  initAccordion() {
    ...
    const descriptionAccordions = [...document.querySelectorAll('.js-description-accordion')];

    window.ResponsiveHelper.addRange({
      '..767': {
        on() {
          new Accordion(descriptionAccordions, {
  ...
  initCurrencySwitcher() {
    ...
    currencySwitcher && observer.observe(currencySwitcher);
  }
  ...
  customCloseDetails() {
    const mainDetails = document.getElementById('filter-main-open-close');
    const mainDetailsPredictiveSearch = document.getElementById('predictive-search');
    const closeBtn = mainDetails?.querySelector('[data-close-click]');
    const applyCloseBtn = mainDetails?.querySelector('[data-apply-close-click]');
    const outsideCloseBtn = mainDetails?.querySelector('[data-outside-close-click]');
    const closeBtnPredictiveSearch = mainDetailsPredictiveSearch?.querySelector('[data-close-predictive-search]');

    function handleButtonClick(e) {
      e.preventDefault();

      if (mainDetails && mainDetails.hasAttribute('open')) {
        mainDetails.removeAttribute('open');
      } else if (mainDetailsPredictiveSearch && mainDetailsPredictiveSearch.hasAttribute('open')) {
        mainDetailsPredictiveSearch.removeAttribute('open');
      }
    }

    closeBtn?.addEventListener('click', handleButtonClick);
    applyCloseBtn?.addEventListener('click', handleButtonClick);
    outsideCloseBtn?.addEventListener('click', handleButtonClick);
    closeBtnPredictiveSearch?.addEventListener('click', handleButtonClick);
  }
  ...
  cartDrawer() {
    ...
    window.addEventListener('productAddedOpenDrawer', openCartDrawer);
